...
import com.free.wallpaper.download.activities.CollectionListActivity;
import com.free.wallpaper.download.adapters.CollectionAdapter;
...
public class CollectionsFragment extends Fragment implements PaginationAdapterCallback, ClickListener {
    ...
    Button btnRetry;
    TextView txtError;
    private ArrayList<Collection> featuredCollections;
    int pageLoadCount = 0;
    ...
    private void populateGrid() {
        adapter = new CollectionAdapter(getContext(), featuredCollections, this,this, Glide.with(getActivity().getApplicationContext()));
    ...
    private void displayData() {
        if (featuredCollections != null) {
            ...
            Call<List<Collection>> call = apiService.getFeaturedCollections(currentPage, Util.PER_PAGE,
                    Util.APPLICATION_ID);
            call.enqueue(new Callback<List<Collection>>() {
                ...
                public void onResponse(Call<List<Collection>> call, Response<List<Collection>> response) {
                    if(response.code()==Util.RESPONSE_RATE_LIMIT_EXCEEDED){
                        ...
                        featuredCollections =  new ArrayList<>(response.body());
                        populateGrid();
                    }
...
//                getResources().getDimensionPixelSize(R.dimen.grid_item_spacing)));
    ...
    private void loadNextPage() {
        ...
        UnsplashApiService apiService =
                UnsplashApiClient.getClient().create(UnsplashApiService.class);
        Call<List<Collection>> call = apiService.getFeaturedCollections(currentPage, Util.PER_PAGE,Util.APPLICATION_ID);
        call.enqueue(new Callback<List<Collection>>() {
            @Override
            public void onResponse(Call<List<Collection>> call, Response<List<Collection>> response) {
                if(response.code()==Util.RESPONSE_RATE_LIMIT_EXCEEDED){
                   // showErrorView(new Throwable());
                    ...
                    isLoading = false;
                    ArrayList<Collection> results = new ArrayList<>(response.body());
    ...
    public void onItemClicked(int position) {
        ...
        Collection current = featuredCollections.get(position);
